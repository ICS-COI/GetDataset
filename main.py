...
if __name__ == '__main__':
    ...
    result_folder = os.path.join(data_root, r'result/F-actin_from_lake')
    lake_path = r"D:\Files\OneDrive - stu.hit.edu.cn/codes\python/MSIM-MPSS-tiff/data/241211-different sigma/example/lake.tif"
    ...
    image_shape = [224, 256, 256]
    image = np.zeros(image_shape)
    lattice_vectors = [np.array([18. * 2, 0.]), np.array([-9. * 2, - 15.5884572 * 2]),
                       np.array([-9. * 2, 15.5884572 * 2])]
    ...
    simulate_degrade.simulate_degrade(
        ...
        blur_params=params, show_steps=True, save=False, save_gt=False
    )

    # simulate_degrade.simulate_degrade_from_lake(lake_path=lake_path, filepath_list=filepath_list,
    #                                             result_folder=result_folder, show_steps=True, save=True, save_gt=True)
